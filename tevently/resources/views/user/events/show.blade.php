    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8 order-2 lg:order-1">
            ...
            @if(isset($similarEvents) && $similarEvents->count() > 0)
                <div class="bg-white rounded-3xl shadow-xl p-6 border border-gray-100">
                    <h2 class="text-xl font-bold text-custom-dark mb-4 border-b pb-3">Event Serupa Lainnya</h2>
                    <div class="space-y-4">
        ...
        <div class="lg:col-span-1 order-1 lg:order-2 space-y-8">
            <div class="bg-white rounded-3xl shadow-xl p-6 border border-gray-100 sticky top-4">
                <h2 class="text-xl font-bold text-custom-dark mb-6 border-b pb-3">Pilihan Tiket</h2>
                ...
                @if($event->tickets->count() > 0)
                    <div class="space-y-4">
                        @foreach($event->tickets as $ticket)
                            ...
                            <div class="border rounded-xl p-4 transition {{ $canBuy ? 'border-main-purple hover:shadow-lg' : 'border-gray-200 bg-gray-50' }}">
                                <div class="flex justify-between items-start mb-2">
                                    <div>
                                        <h4 class="font-bold text-lg text-custom-dark">{{ $ticket->name }}</h4>
                                        <p class="text-gray-600 text-xs mt-1">{{ $ticket->description }}</p>
                                    </div>
                                    <span class="text-xl font-extrabold text-main-purple flex-shrink-0 ml-3">
                                        Rp {{ number_format($ticket->price, 0, ',', '.') }}
                                    </span>
                                </div>
                                
                                <div class="text-xs text-gray-600 space-y-1 mt-3 pt-3 border-t border-gray-100">
                                ...
                                <div class="mt-4">
                                    @if($canBuy)
                                        ...
                                        class="block w-full text-center bg-pink-accent text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-[#f6a5c0] transition shadow-md">
                                            Pesan Tiket
                                        </a>
                                    @else
                                        <button disabled class="block w-full text-center bg-gray-300 text-gray-600 px-4 py-2 rounded-xl text-sm font-bold cursor-not-allowed">
                ...
                @else
                    <p class="text-gray-500 text-center py-4 text-sm">Tidak ada tiket yang terdaftar untuk acara ini.</p>
                @endif
